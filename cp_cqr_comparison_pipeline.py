...
from quantile_regression.quantile_experiment_data_generation import load_histories as load_qr_histories
...
def generate_comparison_analysis(cp_hist_name: str, cqr_hist_name: str, normalize_cp: bool = True):
    """
    ...
    # CP splits
    cp_truth_calib, cp_truth_test, cp_qpens_calib, cp_qpens_test, cp_cnn_calib, cp_cnn_test = train_test_split(
        cp_truth, cp_qpens, cp_cnn, test_size=test_size, random_state=random_state
    )
    
    # CQR splits
    cqr_truth_calib, cqr_truth_test, cqr_qpens_calib, cqr_qpens_test, cqr_lower_calib, cqr_lower_test, cqr_upper_calib, cqr_upper_test = train_test_split(
        cqr_truth, cqr_qpens, cqr_lower, cqr_upper, test_size=test_size, random_state=random_state
    )
    
    # Run CP (standard)
    cp_quantiles = calibrate(cp_qpens_calib, cp_cnn_calib, normalization_term=1)
    cp_cnn_mean = np.mean(cp_cnn_test, axis=-1)
    cp_quantiles_exp = np.tile(np.expand_dims(cp_quantiles, (0, -1)), 
    ...
    if normalize_cp:
        ...
        cp_norm_quantiles_exp = np.tile(np.expand_dims(cp_norm_quantiles, (0, -1)), 
                                       (cp_cnn_mean.shape[0], 1, 1, cp_cnn_mean.shape[-1])) * cp_test_std
    ...
    # Run CQR
    cqr_adjustments = calibrate_quantile_intervals_symmetric(cqr_qpens_calib, cqr_lower_calib, cqr_upper_calib)
    cqr_lower_adj, cqr_upper_adj = apply_symmetric_quantile_adjustments(cqr_lower_test, cqr_upper_test, cqr_adjustments)
    ...
    plot_interval_width_comparison(intervals, methods, f"{cp_hist_name}_vs_{cqr_hist_name}")
...
def plot_coverage_comparison(coverages, method_names, save_name):
    ...
    for i, var_name in enumerate(variable_names):
        for j, (coverage, method_name) in enumerate(zip(coverages, method_names)):
            ...
            ax.plot(timesteps, coverage_mean, 'b-', linewidth=2, label='Actual Coverage')
...
def plot_interval_width_comparison(intervals, method_names, save_name):
    ...
    for i, var_name in enumerate(variable_names):
        for j, ((lower, upper), method_name) in enumerate(zip(intervals, method_names)):
            ...
            # Std over seeds
            widths_std = np.std(np.mean(interval_widths[:, :, i, :], axis=-1), axis=0)  # Shape: (timesteps,)
