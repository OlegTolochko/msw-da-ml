...
def train_rf(training_data_name: str, cnn_model_name: str):
    ...
    data = DataGenerationPipeline.load_pipeline_state(pipeline_state_name=training_data_name)
    ...
    model, norm_stats = load_trained_model(load_model_name=cnn_model_name, device=device)
    model.eval()
    ...
    X = kf_flat[:, :qp_data.shape[1], :].transpose(0, 2, 1).reshape(-1, qp_data.shape[1])
    Y = residuals.transpose(0, 2, 1).reshape(-1, residuals.shape[1])
    
    print(f"Training RF on {X.shape[0]} samples")

    rf = RandomForestRegressor(n_estimators=100, min_samples_leaf=5, n_jobs=-1, verbose=1)
    rf.fit(X, Y)
