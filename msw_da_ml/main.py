...
@app.command()
def generate_conformal_prediction_data(
    load_model_name: str = ""
):
...
def compare_cp_vs_cqr(
    ...
    """
    generate_comparison_analysis(cp_hist_name, cqr_hist_name, normalize_cp)
...
def list_available_models():
    ...
    if cnn_model_dir.exists():
        models = [f for f in cnn_model_dir.iterdir() if f.is_file() and f.suffix == '.pth']
        if models:
    ...
    qr_model_dir = get_output_dir(settings.global_config.trained_quantile_nn_model_out_filename)
    if qr_model_dir.exists():
        models = [f for f in qr_model_dir.iterdir() if f.is_file() and f.suffix == '.pth']
        if models:
            for model in sorted(models, key=lambda x: x.stat().st_mtime, reverse=True):
                print(f"  {model.name}")
        else:
...
def list_available_data():
    ...
    if training_data_dir.exists():
        data_files = [f for f in training_data_dir.iterdir() if f.is_file() and f.suffix == '.pkl']
        if data_files:
    ...
    exp_data_dir = get_output_dir(settings.global_config.experiment_histories_out_filename)
    if exp_data_dir.exists():
        exp_files = [f for f in exp_data_dir.iterdir() if f.is_file() and f.suffix == '.npz']
        if exp_files:
            for exp_file in sorted(exp_files, key=lambda x: x.stat().st_mtime, reverse=True):
    ...
    exp_data_dir = get_output_dir(settings.global_config.quantile_experiment_histories_out_filename)
    if exp_data_dir.exists():
        exp_files = [f for f in exp_data_dir.iterdir() if f.is_file() and f.suffix == '.npz']
        if exp_files:
            for exp_file in sorted(exp_files, key=lambda x: x.stat().st_mtime, reverse=True):
